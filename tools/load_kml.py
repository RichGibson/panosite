...
BASE_URL =  'http://gigapan.com/gigapans/'
items = 2000
page=1
per_page=60
username="rich"
...
for g in gigapans:
    if g.latitude != 0:    
        filename = "%i.kml" % (g.gigapan_id)
        url = "http://gigapan.com/gigapans/%s" % (filename)
        out_file = "static/kml/%s" % filename

        # check if filename exists
        try:
           print "check for  %s" % (  g.name)
           with open(out_file) as f: pass
        except IOError as e:
           print "\tfetching kml %s for  %s" % ( url, g.name)
           url = "http://gigapan.com/gigapans/%s" % (filename)

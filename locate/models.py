...
class GenericTaggedItemBase(ItemBase):
    ...
    @classmethod
    def location_for(cls, model, instance=None):
        ct = ContentType.objects.get_for_model(model)
        kwargs = {
            "%s__content_type" % cls.location_relname(): ct
        }
        if instance is not None:
            kwargs["%s__object_id" % cls.location_relname()] = instance.pk
        return cls.location_model().objects.filter(**kwargs).distinct()


class LocationItem(GenericLocationItemBase, LocationItemBase):
    class Meta:
